...
from tkinter import *
from controller import ControllerModificaImpiego


class ViewModificaImpiego:
    def __init__(self, lavoratori_reg, lavoratore, listbox_lavori, datilavoro, listbox_selection, lista_file):
        ...
        nomeAzienda, ubicazione, iniziostring, finestring, retribuzione, impieghi = datilavoro

        # inizio dichiarazione entry
        self.entry_nomeazienda = customtkinter.CTkEntry(master=self.frame_nomeazienda,
                                                        placeholder_text="Inserisci azienda")
        self.entry_nomeazienda.insert(0, nomeAzienda)

        self.entry_ubicazione = customtkinter.CTkEntry(master=self.frame_ubicazione, placeholder_text="Inserisci ubicazione")
        self.entry_ubicazione.insert(0, ubicazione)

        self.entry_initwork = customtkinter.CTkEntry(master=self.frame_initwork, placeholder_text="gg/mm/aaaa")
        self.entry_initwork.insert(0, iniziostring)

        self.entry_endwork = customtkinter.CTkEntry(master=self.frame_endwork, placeholder_text="gg/mm/aaaa")
        self.entry_endwork.insert(0, finestring)

        self.entry_retribuzione = customtkinter.CTkEntry(master=self.frame_retribuzione, placeholder_text="€€€€")
        self.entry_retribuzione.insert(0, retribuzione)

        self.entry_impiego = customtkinter.CTkEntry(master=self.frame_impiego, width=300,
                                                    placeholder_text="Inserisci le mansioni separate da virgola")
        self.entry_impiego.insert(0, ','.join(impieghi))
        ...
        self.button_inserisci = customtkinter.CTkButton(master=self.frame1, text="Conferma Modifiche",
                                                        command=lambda: ControllerModificaImpiego.ControllerModificaImpiego.mod_job(
                                                            lavoratori_reg, lavoratore, dati, listbox_lavori, self.window, listbox_selection, lista_file))
